...
describe('argy.parse()', function() {
	...
	it.only('should pass though correctly', function() {
	...
	it('should parse basic tests correctly', function() {
		...
		expect(argy([123]).as('[string] number').parse()).to.deep.equal([undefined, 23]);
	});

	it('should accept optional arguments correctly', function() {
		expect(argy(['foo', undefined]).as('string [string]').parse()).to.be.deep.equal(['foo', undefined]);
		expect(argy(['foo', undefined, 123]).as('string [string] [number]').parse()).to.be.deep.equal(['foo', undefined, 123]);
	});

	it.only('should process basic scenarios #1', function() {
	...
	it.skip('should process basic scenarios #2', function() {
		expect(argy(['Joe']).as('[string] [array|string]').parse()).to.deep.equal(['Joe', undefined]);
